  ...
  METHOD call_journal_entry.
    ...
    LOOP AT ct_bakiye ASSIGNING FIELD-SYMBOL(<lfs_bakiye>) GROUP BY ( costcenter = <lfs_bakiye>-costcenter ).
      ...
      DATA(lo_destination) = cl_soap_destination_provider=>create_by_comm_arrangement(
        comm_scenario = 'ZFI000_CS_JOURNEYENTRY'"'ZTRM000_CS_JOURNAL_ENTRY_V2'"ZTRM000_CS_JOURNAL_ENTRY_CREAT'"'ZFI000_CS_JOURNEYENTRY'
      ).
...
*
  METHOD call_reverse_journal_entry.
    ...
    LOOP AT ct_bakiye ASSIGNING FIELD-SYMBOL(<lfs_bakiye>) GROUP BY ( costcenter = <lfs_bakiye>-costcenter ).
      ...
      DATA(lo_destination) = cl_soap_destination_provider=>create_by_comm_arrangement(
        comm_scenario = 'ZFI000_CS_JOURNEYENTRY'"'ZTRM000_CS_JOURNAL_ENTRY_V2'"ZTRM000_CS_JOURNAL_ENTRY_CREAT'"ZTRM000_CS_JOURNAL_ENTRY'"'ZFI000_CS_JOURNEYENTRY'
      ).
